...
import { getFirestore, collection, getDocs, getDoc, setDoc, updateDoc, doc, onSnapshot, query, where, addDoc, deleteDoc, writeBatch, enableIndexedDbPersistence } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
...
enableIndexedDbPersistence(db).catch((err) => {
    console.error("Çevrimdışı kalıcılık hatası:", err);
});
...
function switchView(viewName) {
    ...
    if(viewName === 'staff' && currentUser && currentUser.role === 'admin') {
...
async function createNewOrder(tableName) {
    try {
        ...
        batch.update(tableRef, { status: 'active', currentOrderId: orderId, reservedName: '', totalAmount: 0, paidAmount: 0, timestamp: ts });
...
document.getElementById('cancel-edit-btn').addEventListener('click', () => {
    ...
    document.getElementById('save-product-btn').textContent = "ÜRÜN KAYDET";
...
function addItemToOrder(product) {
    ...
    // İşlemi anında başlatıyoruz, sunucu cevabını (await) beklemiyoruz.
    ...
    const newTotal = (currentOrderData.total || 0) + product.price;
    ...
    batch.commit().catch(err => {
        showModal('HATA', 'Ürün eklenemedi. Lütfen tekrar deneyiniz.', '', null, true);
    });
}
...
window.deleteOrderItem = (index) => {
    ...
    if(items[index] && !items[index].deleted) {
        ...
        const newTotal = (currentOrderData.total || 0) - items[index].price;
        currentOrderData.total = newTotal;
        ...
        batch.update(doc(db, "orders", currentOrderDocId), { items: items, logs: logs, total: newTotal });
        batch.update(doc(db, "tables", currentTableId), { totalAmount: newTotal });
        batch.commit().catch(err => showModal('HATA', 'Ürün silinemedi.', '', null, true));
    }
};
...
window.addNoteToItem = (orderId, index, currentNote) => {
    ...
    `, (data) => {
        ...
        if(items[index]) {
            ...
            updateDoc(doc(db, "orders", orderId), { items: items }).catch(err => showModal('HATA', 'Not eklenemedi.', '', null, true));
        }
    });
};
...
document.getElementById('partial-pay-btn').addEventListener('click', () => {
    ...
    `, (data) => {
        ...
        if(amt > 0) {
            ...
            }).catch(err => showModal('HATA', 'Ödeme alınamadı.', '', null, true));
        }
    });
});
...
document.getElementById('item-pay-btn').addEventListener('click', () => {
    ...
    showModal('ÜRÜN BAZLI ÖDEME', 'Ödenecek ürünleri seçiniz:', checklistHtml, (data) => {
        ...
        if(totalToPay > 0) {
            ...
            }).catch(err => showModal('HATA', 'Ödeme başarısız.', '', null, true));
        }
    });
...
document.getElementById('close-table-btn').addEventListener('click', () => {
    ...
    if(remaining > 0) {
        ...
        `, (data) => {
            ...
            const batch = writeBatch(db);
            batch.update(doc(db, "orders", currentOrderDocId), { partialPayments: pays, paid: d.total, logs: logs, status: 'closed', closedAt: new Date().toISOString() });
            ...
            }).catch(err => showModal('HATA', 'Masa kapatılamadı.', '', null, true));
        });
    } else {
        showModal('MASAYI KAPAT', 'AÇIK HESAP BULUNMUYOR. MASA BOŞALTILACAKTIR. ONAYLIYOR MUSUNUZ?', '', () => {
            ...
            const batch = writeBatch(db);
            batch.update(doc(db, "orders", currentOrderDocId), { status: 'closed', logs: logs, closedAt: new Date().toISOString() });
            batch.update(doc(db, "tables", currentTableId), { status: 'empty', currentOrderId: null, totalAmount: 0, paidAmount: 0, reservedName: '', reservedColor: null, timestamp: null });
            ...
            }).catch(err => showModal('HATA', 'Masa boşaltılamadı.', '', null, true));
        });
    }
});
